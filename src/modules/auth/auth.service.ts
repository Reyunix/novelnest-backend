...
export const createUser = async (rawData: unknown) => {
  ...
  const { userAlreadyExists, emailAlreadyExists } = await checkIfUserExists(userData);
...
export const validateLoginCredentials = async (rawData: unknown): Promise<AuthSafeUser> => {
